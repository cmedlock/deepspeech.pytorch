...
def _collate_fn(batch): # batch is a list of 20 (spectrogram, transcription) tuples
    def func(p):
        return p[0].size(1)
    batch = sorted(batch, key=lambda sample: sample[0].size(1), reverse=True)
    # identify longest audio recording in the batch
    longest_sample = max(batch, key=func)[0] # the spectrogram with the most dft's
    freq_size = longest_sample.size(0) # number of frequency bins
    minibatch_size = len(batch)
    max_seqlength = longest_sample.size(1) # number of dft's
    inputs = torch.zeros(minibatch_size, 1, freq_size, max_seqlength) # concatenation of 20 spectrograms
    input_percentages = torch.FloatTensor(minibatch_size) # list of length/max_length
    target_sizes = torch.IntTensor(minibatch_size) # list of transcription lengths
    targets = [] # concatenation of batch_size transcriptions
    for x in range(minibatch_size):
        sample = batch[x] # (spectrogram, transcription) pair
        tensor = sample[0] # spectrogram
        target = sample[1] # transcription
        seq_length = tensor.size(1) # number of dft's
        inputs[x][0].narrow(1, 0, seq_length).copy_(tensor)
        input_percentages[x] = seq_length / float(max_seqlength)
        target_sizes[x] = len(target) # length of transcription
